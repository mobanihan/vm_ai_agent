...
class WebSocketCommandHandler:
    ...
    async def connect(self):
        ...
        while self._running:
            try:
                # Ensure we have credentials before connecting
                vm_id = self.security_manager.get_vm_id()
                api_key = self.security_manager.get_api_key()
                
                if not vm_id or not api_key:
                    logger.error("VM ID or API key not available for WebSocket connection")
                ...
                ws_url = f"{self.orchestrator_url}/api/v1/agents/{vm_id}/ws"
    ...
    async def _send_heartbeat(self):
        ...
        vm_id = self.security_manager.get_vm_id()
        if not vm_id:
            logger.warning("Cannot send heartbeat: VM ID not available")
            return
    ...
    async def register_agent(self, provisioning_token: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if not vm_id or not api_key:
                logger.error("Cannot register: VM ID or API key not available")
                return False
            ...
            registration_data = {
                ...
                "capabilities": {
                    "shell_executor": 'shell' in self.vm_agent.tools,
                    "file_manager": 'file' in self.vm_agent.tools,
                    "system_monitor": 'system' in self.vm_agent.tools,
                    "log_analyzer": 'log' in self.vm_agent.tools
                }
            }
            
            # If we have certificates, include CSR for renewal
            if self.security_manager.vm_csr_path.exists():
                with open(self.security_manager.vm_csr_path, 'r') as f:
                    registration_data["csr"] = f.read()
            ...
            async with aiohttp.ClientSession() as session:
                ...
                # Use HTTP for initial registration, HTTPS for subsequent communications
                registration_url = self.orchestrator_url.replace("wss://", "https://").replace("ws://", "http://")
                registration_url = f"{registration_url}/api/v1/agents/register"
                
                async with session.post(
                    registration_url,
                    json=registration_data,
                    headers=headers,
                    ssl=False  # Initial registration might not have proper certs
                ) as response:
                    if response.status == 200:
                        result = await response.json()
                        
                        # Save any certificates provided
                        if "certificate" in result:
                            with open(self.security_manager.vm_cert_path, 'w') as f:
                                f.write(result["certificate"])
                            logger.info("Received and saved agent certificate")
                        
                        if "ca_certificate" in result:
                            with open(self.security_manager.ca_cert_path, 'w') as f:
                                f.write(result["ca_certificate"])
                            logger.info("Received and saved CA certificate")
                        
                        # Update SSL context if certificates were provided
                        if "certificate" in result or "ca_certificate" in result:
                            self.security_manager._ssl_context = self.security_manager._create_ssl_context()
                        
                        logger.info(f"Successfully registered agent {vm_id}")
                        return True
                    else:
